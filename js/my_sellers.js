...
async function loadInitialData(user_id) {
    ...
    try {
        const [_userMap, sellersRes, favoritesRes] = await Promise.all([
            ...
            _supabase.from('sellers').select('*, companies(*)').eq('user_id', user_id).is('deleted_at', null),
            _supabase.from('favorites').select('item_id, created_at').eq('user_id', user_id).eq('item_type', 'seller').order('created_at', { ascending: false })
        ]);
        userMap = _userMap;
...
function renderSellers() {
    ...
    pageItems.forEach(seller => {
        ...
        const createdStr = fmtDate(seller.created_at);
        const updatedStr = fmtDate(seller.updated_at || seller.created_at);
        ...
        const rowHtml = `
            <tr onclick="showSellerDetail('${seller.id}')" style="cursor: pointer;">
                ...
                <td style="padding: 20px 24px !important; border-right: 1px solid #f8fafc;">
                    <span class="industry-tag-td" style="background: ${isDraft ? '#f1f5f9' : '#f5f3ff'}; color: ${isDraft ? '#94a3b8' : '#8b5cf6'}; border: 1px solid ${isDraft ? '#e2e8f0' : '#ddd6fe'};">${escapeHtml((seller.industry || "기타").replace(/^기타:\s*/, ''))}</span>
                </td>
