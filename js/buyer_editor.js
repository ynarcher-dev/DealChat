...
$(document).ready(function () {
    ...
    function loadBuyerData() {
        ...
        APIcall(getPayload, endpoint, { 'Content-Type': 'application/json' })
            ...
            .then(data => {
                ...
                // ??梨꾩슦湲?                $('#buyer-name-editor').text(buyer.companyName || '');
                document.title = `${buyer.companyName || '諛붿씠??} - 諛붿씠???뺣낫`;
                $('#sidebar-header-title').text(buyer.companyName || '諛붿씠???뺣낫');
                $('#buyer-industry').val(buyer.interest_industry || '?좏깮?댁＜?몄슂');
                ...
                // ?좎쭨 諛??묒꽦???뺣낫 諛붿씤??                if (buyer.user_id) {
                    APIcall({
                        action: 'get',
                        table: 'users',
                        id: buyer.user_id
    ...
    // ==========================================
    // ?뚯씪 ?낅줈??/ 紐⑸줉 (buyers_files ?뚯씠釉??ъ슜 媛??
    // ==========================================
    ...
    $(document).on('click', '.btn-remove-file', function () {
        ...
        APIcall({ action: 'delete', table: 'buyers_files', id: fileId }, SUPABASE_ENDPOINT, { 'Content-Type': 'application/json' })
            .then(r => r.json())
