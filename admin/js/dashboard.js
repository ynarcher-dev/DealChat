...
async function loadPage(page) {
    ...
    switch (page) {
        case 'dashboard':
            ...
            break;
        case 'users':
            await renderGrid($content, 'users', '?åÏõê Í¥ÄÎ¶?);
            break;
        case 'companies':
            await renderGrid($content, 'companies', 'Í∏∞ÏóÖ Í¥ÄÎ¶?);
            break;
        case 'files':
            await renderGrid($content, 'files', '?åÏùº Í¥ÄÎ¶?);
            break;
        case 'reports':
            await renderGrid($content, 'reports', 'Î≥¥Í≥†???§Ï†ï');
            break;
        case 'qna':
            await renderGrid($content, 'qna', '?ÅÎã¥ Î¨∏Ïùò');
            break;
        case 'sellers':
            await renderGrid($content, 'sellers', 'Îß§ÎèÑ Îß§Î¨º');
            break;
        case 'buyers':
            await renderGrid($content, 'buyers', 'Îß§Ïàò ?¨Îßù');
            break;
        default:
            renderDashboard($content);
    }
        ...
        <div class="data-grid-container">
            <div class="grid-header">
                ...
                <div class="actions-toolbar">
                    <button class="btn-sm btn-outline" id="refresh-btn">?àÎ°úÍ≥†Ïπ®</button>
                    <button class="btn-sm btn-outline" style="color:red;" id="delete-btn">??†ú (Delete)</button>
                </div>
            </div>
...
async function deleteSelectedRows(gridOptions, tableName) {
    ...
    if (!confirm(`?†ÌÉù??${selectedNodes.length}Í∞???™©???ïÎßê ??†ú?òÏãúÍ≤†Ïäµ?àÍπå?`)) return;
    ...
    alert(`${successCount}Í∞???™© ??†ú Ï≤òÎ¶¨ ?ÑÎ£å (?úÎ≤Ñ ÏßÄ???¨Î? ?ïÏù∏ ?ÑÏöî)`);
    loadPage(tableName);
}
