...
function exportToCSV() {
    ...
    const rows = filteredSellers.map(s => {
        ...
        const _sd = new Date(s.created_at);
        const date = `${_sd.getFullYear()}.${String(_sd.getMonth()+1).padStart(2,'0')}.${String(_sd.getDate()).padStart(2,'0')}`;
...
function renderNdaTimeline() {
    ...
    items.forEach(item => {
        ...
        const _sd2 = new Date(item.created_at);
        const signedDate = `${_sd2.getFullYear()}.${String(_sd2.getMonth()+1).padStart(2,'0')}.${String(_sd2.getDate()).padStart(2,'0')} ${String(_sd2.getHours()).padStart(2,'0')}:${String(_sd2.getMinutes()).padStart(2,'0')}`;
        const _vd = item.viewed_at ? new Date(item.viewed_at) : null;
        const viewedDate = _vd
            ? `${_vd.getFullYear()}.${String(_vd.getMonth()+1).padStart(2,'0')}.${String(_vd.getDate()).padStart(2,'0')} ${String(_vd.getHours()).padStart(2,'0')}:${String(_vd.getMinutes()).padStart(2,'0')}`
