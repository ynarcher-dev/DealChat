...
function renderCompanies() {
    ...
    items.forEach(c => {
        const _d = new Date(c.updated_at || c.created_at);
...
window.showCompanyDetail = function (id) {
    ...
    if (!c) return;
    const author = userMap[c.user_id] || { name: 'Unknown', affiliation: 'DealChat' };
    const dateStr = `?깅줉?? ${(d => `${d.getFullYear()}.${String(d.getMonth()+1).padStart(2,'0')}.${String(d.getDate()).padStart(2,'0')}`)(new Date(c.created_at))}`;
...
function renderInbox() {
    ...
    items.forEach(item => {
        ...
        const _id = new Date(item.created_at);
        const date = `${_id.getFullYear()}.${String(_id.getMonth()+1).padStart(2,'0')}.${String(_id.getDate()).padStart(2,'0')} ${String(_id.getHours()).padStart(2,'0')}:${String(_id.getMinutes()).padStart(2,'0')}`;
...
function exportToCSV() {
    ...
    const rows = filteredCompanies.map(c => {
        ...
        return [
            ...
            userMap[c.user_id]?.name || 'Unknown', (d => `${d.getFullYear()}.${String(d.getMonth()+1).padStart(2,'0')}.${String(d.getDate()).padStart(2,'0')}`)(new Date(c.updated_at || c.created_at))
